/*
*	Program:	s1.c
*   Author:		LiKunhao Jingjiyao LiRan LiuTao ZhangYuhao
*   Date:		Oct 18, 2017
*	Version:     	1.2
*	
*	Compilation and link options : (Linux)          
*		gcc s1.c -lnsl -o s1 or
*
*		gcc s1.c -lnsl -w -o s1 
...
#define BUFFSIZE	256
#define SERVER_NAME	"Mini web server  V1.2 (LK_Hao)"
...
/* ***************************************************************************
*	find_extension 1=html 2=jpg 3=gif 4=PNG
*  ***************************************************************************
*/
...
int find_extension(char *extension)
{
	...
	{
		if((strstr(extension ,".png") != NULL) || (strstr(extension ,".PNG") != NULL))
		{
			type_int = 4 ;
		}
...
/* Disable access to the previous folder */		
	if(strstr(path,"/..") != NULL)
		{
			send_header(conn,400,strlen(ERROR_400) ,strlen(ERROR_400));
			send(conn,ERROR_400,strlen(ERROR_400),0);
		...
		/* inital folder name ,all page should be in html folder*/
		char filename1[BUFFSIZE] = "html"; 
		/*most important  use pwd to find the path !!*/
		/*char filename1[BUFFSIZE] = "/home/tt";*/
		...
		if(type_int1 == 3) //gif
		{
			...
			send_header(conn, 200,T_GIF,buf.st_size); /*send header gif*/
		...
		if(type_int1 == 4) //png
		{
			...
			if(stat(filename1, &buf) != 0)
			{ 
				perror("error image.png");
			}
			if((ptr = fopen(filename1,"r")) == NULL)
			{ 
				perror("error / open image.png");
				//exit(0);
			} 
			send_header(conn, 200 ,T_PNG ,buf.st_size);
			printf("\n File size : %d octets \n", buf.st_size);
			while(!feof(ptr))
			{
				rval=fread(buff,sizeof(char),BUFFSIZE,ptr);
